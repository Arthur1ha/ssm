...
def _insert_episode(episodes_dir: Path, ts: float, content: str) -> None:
    """在 JSONL 文件中插入一条记录。"""
    episodes_dir.mkdir(parents=True, exist_ok=True)
    today = datetime.fromtimestamp(ts).strftime("%Y-%m-%d")
    f = episodes_dir / f"{today}.jsonl"
    entry = {"ts": ts, "event_type": "ACTION_TAKEN", "content": content}
...
def test_get_episodes_for_date_returns_todays_records(tmp_path):
    ...
    _insert_episode(episodes_dir, time.time(), "今天探索了走廊")
    episodes = _get_episodes_for_date(datetime.now().strftime("%Y-%m-%d"), episodes_dir=episodes_dir)
...
def test_get_episodes_for_date_excludes_other_days(tmp_path):
    ...
    _insert_episode(episodes_dir, yesterday_ts, "昨天的事情")
    today_str = datetime.now().strftime("%Y-%m-%d")
    episodes = _get_episodes_for_date(today_str, episodes_dir=episodes_dir)
...
async def test_generate_and_save_returns_none_when_no_episodes(tmp_path):
    ...
    episodes_dir = tmp_path / "episodes"
    episodes_dir.mkdir(parents=True, exist_ok=True)
    result = await generate_and_save("2026-01-01", db_path=tmp_path / "t.db", episodes_dir=episodes_dir)
...
async def test_generate_and_save_calls_llm_and_persists(tmp_path):
    ...
    episodes_dir = tmp_path / "episodes"
    today_str = datetime.now().strftime("%Y-%m-%d")
    _insert_episode(episodes_dir, time.time(), "探索了北边区域")
    ...
    with patch("cloud.go2.agentcore.memory.daily_summary.get_text_llm") as mock_llm_fn, \
        ...
        result = await generate_and_save(today_str, db_path=db, episodes_dir=episodes_dir)
...
async def test_ensure_yesterday_summary_skips_if_already_exists(tmp_path):
    ...
    with patch("cloud.go2.agentcore.memory.daily_summary.generate_and_save") as mock_gen:
        await ensure_yesterday_summary(db_path=db, episodes_dir=episodes_dir)
